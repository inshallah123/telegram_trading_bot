...
def generate_contract_names(symbol: str) -> List[str]:
    """
    生成期货合约名称列表
    ...
    Returns:
        合约名称列表，格式为 品种英文+YYMM.交易所英文
        - 普通品种：从下个月开始连续9个月的合约
        - 中金所品种：从下个季度月开始连续4个季度合约

    Raises:
        ValueError: 当品种代码不存在时

    Examples:
        >>> # 假设当前为2025年5月
        >>> generate_contract_names("RM")  # 菜粕，郑商所  # doctest: +SKIP
        ['RM2506.CZC', 'RM2507.CZC', 'RM2508.CZC', 'RM2509.CZC', 'RM2510.CZC', 'RM2511.CZC', 'RM2512.CZC', 'RM2601.CZC', 'RM2602.CZC']

        >>> generate_contract_names("IF")  # 沪深300，中金所  # doctest: +SKIP
        ['IF2506.CFE', 'IF2509.CFE', 'IF2512.CFE', 'IF2603.CFE']
    """
    if symbol not in CONTRACTS_DATA:
        raise ValueError(f"品种代码 {symbol} 不存在")

    contract_info = CONTRACTS_DATA[symbol]
    ...
    if exchange == "CFE":
        ...
        for i in range(4):
            yy = str(year)[2:]  # 取年份后两位
            mm = f"{month:02d}"  # 月份补零
            contracts.append(f"{symbol}{yy}{mm}.{exchange}")

            # 计算下一个季度月
            month_index = quarter_months.index(month)
            if month_index == 3:  # 12月后是下一年3月
                year += 1
                month = quarter_months[0]
            else:
                month = quarter_months[month_index + 1]
    else:
        # 普通品种：连续9个月合约，从下个月开始
        year = current_year
        month = current_month + 1  # 从下个月开始
